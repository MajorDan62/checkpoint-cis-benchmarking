...
function ProcessData {
        ...
        if ($_pos_ -gt 0){
                ...
                write-host -f red "No Serial Number Located for $($hostname )"
                $_pos_          =       (Find-Phrase -FilePath $filelocation -Phrase "Platform").linenumber
                $serialnumber    =      "*N/A $(($results[($_pos_)] -split ":")[-1])*"
                }
...
write-host      -b DarkYellow -f black "                                           "
if ($_exop_) { ScreenAlignment 1 4;write-host      -b DarkYellow -f black "[EXOP]";ScreenAlignment 0 5} 
#check if the feeder files exist in current directory
$allExist = $true
...
write-host -f blue "Script was running for $($_delta) 🕒"